...
<div class="top_logo"> <img src="/App/Manage/View/Public/images/main/logo.png" /> </div>
     <div class="nav" id="nav">
      <ul>
      <?php if(is_array($menu)): foreach($menu as $k=>$v): ?><li id="menu_<?php echo ($k); ?>"><a <?php if(empty($k)): ?>class="thisclass"<?php endif; ?> href="#" _for="left_menu_<?php echo ($k); ?>"><b><?php echo ($v["title"]); ?></b></a></li><?php endforeach; endif; ?>
      </ul>
    </div>
	 <div class="top_link">
      <ul>
...
<?php if(is_array($menu)): foreach($menu as $k=>$v): ?><div id="items_left_menu_<?php echo ($k); ?>">
	<?php if(is_array($v["child"])): $k2 = 0; $__LIST__ = $v["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($k2 % 2 );++$k2;?><dl id="dl_items_<?php echo ($k2); ?>_<?php echo ($k); ?>" data-cid="<?php echo ($v2['id']); ?>">
		<dt><?php echo ($v2["title"]); ?></dt>
		<dd>
		<ul>

      <?php if($v2['id'] == 7): if(is_array($qmenu)): $k3 = 0; $__LIST__ = $qmenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v3): $mod = ($k3 % 2 );++$k3;?><li><a href="<?php echo U($v3['name']);?>" target="main"><?php echo ($v3["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>  
      <?php else: ?>
      <?php if(is_array($v2["child"])): $k3 = 0; $__LIST__ = $v2["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v3): $mod = ($k3 % 2 );++$k3;?><li><a href="<?php echo U($v3['name']);?>" target="main"><?php echo ($v3["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
		</ul>
		</dd>
