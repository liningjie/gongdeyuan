...
class SystemController extends CommonController {
	...
	public function add() {
		...
		if (IS_POST) {
			...
			$short_open_tag = ini_get('short_open_tag');//php.ini是否开启php短标签
			...
			if (!$short_open_tag) {
				...
				if (stripos($data['t_value'], '<?') !== false) {
					$data['t_value'] = preg_replace('/<\?(.+?)\?>/i', '', $data['t_value']);
				}
				if (stripos($data['s_value'], '<?') !== false) {
					$data['s_value'] = preg_replace('/<\?(.+?)\?>/i', '', $data['s_value']);
				}

			}

			
			//禁止长风格php代码（php7以下才有长风格php代码）
			//<script\s+language[\s=]+["\']*php["\']*[^>]*?\>.*?<\/script>
			$preg_param = '/<script\s+language[\s=]+["\']*php["\']*[^>]*?\>/is';
			if (stripos($data['t_value'], '<?php') !== false || ($short_open_tag && stripos($data['t_value'], '<?') !== false ) || preg_match($preg_param, $data['t_value'])) {
				$this->error('可选值禁止输入php代码');
			}
			if (stripos($data['s_value'], '<?php') !== false || ($short_open_tag && stripos($data['s_value'], '<?') !== false ) || preg_match($preg_param, $data['s_value'])) {
				$this->error('值禁止输入php代码');
			}
	...
	public function site() {
		if (IS_POST) {
			...
			if (!empty($data['CFG_UPLOAD_IMG_EXT'])) {
				$data['CFG_UPLOAD_IMG_EXT'] = strtolower($data['CFG_UPLOAD_IMG_EXT']);
				$_img_exts = explode(',', $data['CFG_UPLOAD_IMG_EXT']);
				$_all_exts = array('jpg', 'gif', 'png', 'jpeg');
				foreach ($_img_exts as $ext) {
					if (!in_array($ext, $_all_exts)) {
						$this->error('允许上传的图片类型超过范围！只允许填写格式为：jpg,gif,png,jpeg！');
					}
				}
			}
			if (!empty($data['CFG_UPLOAD_FILE_EXT'])) {
				...
				$_no_exts = array('php', 'asp', 'aspx', 'jsp');//'pht','phtml','php3','php4','php5','phps'
				foreach ($_file_exts as $ext) {
					if (in_array($ext, $_no_exts)) {
						$this->error('允许附件类型错误！不允许后缀为：php,asp,aspx,jsp！');
					} elseif (strpos($ext, 'php') === 0 || strpos($ext, 'pht') === 0) {
						$this->error('允许附件类型错误！不允许后缀为：php文件相关类型');
					}
				}
			}
