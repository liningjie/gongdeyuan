...
class SoftController extends CommonContentController {
	...
	public function editPost() {
		...
		if (false !== M('soft')->save($data)) {
			...
			//所属栏目是跟原来栏目是同一个
			if ($selfCate['id'] != $old_cid) {
				$old_nav_cates = Category::getParents(get_category(0), $old_cid);
				if (!empty($old_nav_cates)) {
					$nav_cates = array_merge($nav_cates, $old_nav_cates);
				}
			}
			foreach ($nav_cates as $val) {
				del_cache_html('List/index_' . $val['id'] . '_', false, 'list:index');
	...
	public function audit() {
		...
		if (false !== M('soft')->where(array('id' => array('in', $idArr)))->setField('audit_status', 1)) {

			//更新静态缓存---start
			$nav_cates = Category::getLevelOfModelId(get_category(0), $model_id);
			...
			$this->success('批量审核成功', U('index', array('pid' => $this->pid)));
	...
	public function move() {
		...
		if (IS_POST) {
			...
			if (false !== M($actionName)->where(array('id' => array('in', $id)))->setField('cid', $cid)) {
				...
				foreach ($ids as $val) {
					del_cache_html('Show/index_*_' . $val . '.', false, 'show:index'); //不太精确，会删除其他模块同id文档
				}
				del_cache_html('Index_index', false, 'index:index');
				//更新静态缓存---end
