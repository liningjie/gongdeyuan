...
function is_mobile() {
	$mobile = array();
	$mobile_flag = I('wap', 0, 'intval'); //强制手机
	$mobile_browsers = 'comFront|Mobile|iPhone|Android|Windows Phone|WAP|NetFront|JAVA|Opera Mini|UCWEB|Windows CE|Symbian|Series|webOS|SonyEricsson|Sony|BlackBerry|Cellphone|dopod|Nokia|samsung|PalmSource|Xphone|Xda|Smartphone|PIEPlus|MEIZU|MIDP|CLDC';
	//note 获取手机浏览器
	$agent = $_SERVER['HTTP_USER_AGENT'];
	if (preg_match("/$mobile_browsers/i", $agent, $mobile)) {
		return true;
	} else {
...
function chk_is_bad($str, $flag = 0) {
	...
	if ($flag) {
		$filter = "'|\\b(>|<|=|and|or|in|like|from|union|select|insert|delete|create|alter|drop|truncate|table|database|sleep|ascii|substr|between)\\b";
	}
