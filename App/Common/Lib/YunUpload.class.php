...
class YunUpload {
	...
	public function __construct($thum_flag = 0, $sub_path = 'file1', $file_field = '') {
		...
		//允许类型
		$_allowType = $this->thumFlag ? explode(',', C('CFG_UPLOAD_IMG_EXT')) : explode(',', C('CFG_UPLOAD_FILE_EXT'));
		$_allowType = array_diff($_allowType, array('php', 'asp', 'aspx', 'jsp'));
		//清掉其他不合格的后缀php5,phtml,php:$data
		foreach ($_allowType as $key => $ext) {
			if (strpos($ext, 'php') === 0 || strpos($ext, 'pht') === 0 || strpos($ext, 'asp') === 0 || strpos($ext, 'asph') === 0 || strpos($ext, 'jsp') === 0) {
				unset($_allowType[$key]);
			}
		}

		$this->allowType = array_values($_allowType);
